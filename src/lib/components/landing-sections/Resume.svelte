...
<section id="resume" class="relative py-12 md:py-24 lg:py-36 mb-10 lg:mb-24">
	<div
		class="bg-gray-800 dark:bg-gray-700 absolute top-0 left-0 right-0 bottom-0 transform -skew-y-2 z-00"
	/>

	<div class="container lg:flex justify-between items-start gap-x-6 relative">
		<div class="text-gray-100 lg:pr-20 lg:!sticky !top-48 w-full nightwind-prevent">
			<span
				class="text-xl leading-normal mb-4 pre-line-top-title before:bg-gray-100 nightwind-prevent"
				>Resume</span
			>
			...
			>
				<button
					...
					{currentTab == ActiveTab.Achievements ? 'bg-primary-700 border-primary-700' : 'dark:text-gray-100'}"
					on:click={() => setActiveTab(ActiveTab.Achievements)}>Achievements</button
				>
				<button
					class="rounded-r-2xl border-l-0 w-full py-4 border-2 
					{currentTab == ActiveTab.Experience ? 'bg-primary-700 border-primary-700' : 'dark:text-gray-100'}"
				...
				{#if section.title === currentTab}
					{#each section.items as item}
						<div class="bg-gray-100 p-8 rounded-3xl">
							<span
								class="text-xl leading-normal text-primary-700 before:bg-primary-700 pre-line-top-title"
								>{item.date}</span
							>
							<h4 class="mt-5 mb-6 text-2xl md:text-3xl leading-tight font-semibold">
								{item.title}
							</h4>
